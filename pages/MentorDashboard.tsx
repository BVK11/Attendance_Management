...
import { alertService, leaveService, studentService } from '../services/storageService';
...
const MentorDashboard: React.FC = () => {
    ...
    useEffect(() => {
        if (user?.mentorId) {
            const mentorAlerts = alertService.getAll().filter(a => a.mentorId === user.mentorId);
            setAlerts(mentorAlerts);

            const myStudentRollNos = studentService
                .getAll()
            ...
            setLeaves(
                leaveService
                    .getAll()
                    .filter(l => myStudentRollNos.includes(l.rollNo))
            );
        }
    }, [user, activeTab]);
