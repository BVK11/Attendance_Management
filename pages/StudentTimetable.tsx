...
import { timetableService } from '../services/storageService';
...
const StudentTimetable: React.FC = () => {
    ...
    const timetableByDay = useMemo(() => {
        ...
        const entries = timetableService.getAll()
            .filter(t => t.department === user.department && t.section === user.section);
            
        const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
        const grouped: { [key: string]: any[] } = {};
        
        days.forEach(day => {
            grouped[day] = entries.filter(e => e.day === day).sort((a, b) => a.period - b.period);
        });
